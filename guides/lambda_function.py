...
def launch_request_handler(handler_input):
    ...
    # get the part of the request that contains info on permissions
    system = handler_input.request_envelope.context.system
    
    # do we have permission to get the user's first name?
    if system.user.permissions is None:
        # if not, then we should send back a card asking them to grant permission
        speech_text = "{} needs to know your first name to provide a more personal experience. Please grant it permission to do so in the Alexa app.".format(SKILL_NAME)
        handler_input.response_builder.speak(speech_text).set_card(AskForPermissionsConsentCard(["alexa::profile:given_name:read"])).set_should_end_session(True)
        return handler_input.response_builder.response
    
    # if we do have permission, then we access the customer profile API to get it
    endpoint = system.api_endpoint + "/v2/accounts/~current/settings/Profile.givenName"
    auth = {'Authorization': "Bearer " + system.api_access_token}
    
    # by default the name comes back wrapped in quotes, so we remove them
    fname = requests.get(endpoint, headers=auth).text.replace("\"", "")
...
@sb.request_handler(can_handle_func=is_request_type("System.ExceptionEncountered"))
def exception_encountered_handler(handler_input):
    print("EXC:", handler_input.request_envelope.request)
    return handler_input.response_builder.response
...
def number_guess_handler(handler_input):
    ...
    if guess_num > target_num:
        speech_text = (
            "{} is so high. Try saying a smaller number.".format(guess_num))
...
def fallback_handler(handler_input):
    ...
    if ("game_state" in session_attr and
        ...
        reprompt = "Say yes to start the game or no to quit."
        print("FaBa", handler_input.request_envelope.request)
...
def unhandled_intent_handler(handler_input):
    ...
    print("UnHa", handler_input.request_envelope.request)
